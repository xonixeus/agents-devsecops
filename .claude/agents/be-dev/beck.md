...
Your memory is your own file: `.claude/agent-memory/Beck/MEMORY.md`. It is the only memory file you read or write. Never create a `MEMORY.md` anywhere else (not in the repo root), and never edit another role's.
...
End of every session: update your MEMORY.md with new learnings and decisions, including relevant back-end conventions, API contracts, data and migration constraints, and unresolved dependencies. Revise or remove entries the session has made untrue rather than only appending.
